...
# NEED TO EDIT THIS TO RUN IN SEPERATE SCRIPT AND SUBMIT SCRIPT TO EDDY CUDA IN THIS ONE
...
# Create streamlines
# 100 million tracks recommended.
tckgen -act 5tt_coreg.mif -backtrack -seed_gmwmi gmwmSeed_coreg.mif -nthreads 8 -maxlength 250 -cutoff 0.06 -select 100000000 wmfod_norm.mif tracks_100M.tck

# Extract a subset of tracks (here, 200 thousand) for ease of visualization
tckedit tracks_100M.tck -number 200k smallerTracks_200k.tck

# Reduce the number of streamlines with tcksift
tcksift2 -act 5tt_coreg.mif -out_mu sift_mu.txt -out_coeffs sift_coeffs.txt -nthreads 8 tracks_100M.tck wmfod_norm.mif sift_1M.txt
